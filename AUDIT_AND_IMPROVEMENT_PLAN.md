...
*Update (2026-04-29): Phase 1 of the implementation roadmap (Contract and Safety Fixes) has been largely completed. High-priority issues such as path normalization, schema validation for timeouts, bridge startup error handling, and VSIX artifact hygiene have been addressed.*
...
### Phase 1: Contract and Safety Fixes (✅ Completed)

1. ~~Add `noReopen` to `excel_add_power_query` schema or remove it from public docs/tests.~~ (Added to `package.json` schema)
2. ~~Normalize filesystem paths with `expanduser`/`expandvars` in all path-accepting Python methods.~~ (Implemented in `_norm_path` helper)
3. ~~Add timeout validation in `package.json`.~~ (Minimum 1000, maximum 600000 added)
4. ~~Improve Python path startup errors with `spawn` error handling and actionable messages.~~ (Done in `src/excelBridge.ts`)
5. ~~Update VBA-on-Mac documentation and runtime behavior so they agree.~~ (Detailed runtime exception added on Mac)
6. ~~Tighten `.vscodeignore` and rebuild the VSIX without tests, reports, bytecode, local workbooks, or prior VSIX files.~~ (Excluded in `.vscodeignore`)
